...
uses
  ...
  FMX.Types, FMX.Controls, FMX.TextLayout, System.Math.Vectors, System.Rtti,
  FMX.Objects,
...
type
  ...
  TListItemCallback = TProc<TListItemView, TListItemDrawable,
    TListItemCallbackOp>;
  ...
  protected type
    TStyleResource = (FontSize, FontFamily, FontStyle, TextColor,
      SelectedTextColor, TextShadowColor, PressedTextColor);
    TStyleResources = set of TStyleResource;
  protected const
    TextResources: set of TStyleResource = [TStyleResource.FontFamily,
      TStyleResource.FontSize, TStyleResource.FontStyle,
      TStyleResource.TextColor, TStyleResource.TextShadowColor,
  ...
  protected
    ...
    procedure DoUpdateValuesFromResources(const Resources
      : TListItemStyleResources; const Purpose: TListItemPurpose); virtual;
  public
    ...
    procedure Render(const Canvas: TCanvas; const DrawItemIndex: Integer;
      ...
      const Resources: TListItemStyleResources; const Params: TParams;
    ...
    procedure UpdateValuesFromResources(const Resources
      : TListItemStyleResources; const Purpose: TListItemPurpose);
  ...
  protected
    ...
    procedure DoUpdateValuesFromResources(const Resources
      : TListItemStyleResources; const Purpose: TListItemPurpose); override;
  ...
  public
    ...
    property SelectedTextColor: TAlphaColor read FSelectedTextColor
    ...
    property TextShadowColor: TAlphaColor read FTextShadowColor
  ...
  const
    ...
    smOriginal = TImageScalingMode.Original deprecated
      'Use TImageScalingMode.Original';
    smStretch = TImageScalingMode.Stretch deprecated
      'Use TImageScalingMode.Stretch';
  end;
  ...
  public
    ...
    procedure FitInto(const Bitmap: TBitmap;
      var InputRect, DestinationRect: TRectF);
    property SrcRect: TRectF read FSrcRect write SetSrcRect;
    property ScalingMode: TImageScalingMode read FImageScalingMode
      write SetImageScalingMode default TImageScalingMode.StretchWithAspect;
  ...
  public
    ...
    property AccessoryType: TAccessoryType read FAccessoryType
      write SetAccessoryType;
  end;
  ...
  private
    ...
    procedure SetInvalidateCallback(const Callback: TListItemCallback);
      override;
  protected
  ...
  public
    ...
    property TextVertAlign: TTextAlign read GetTextVertAlign
      write SetTextVertAlign;

    property WordWrap: Boolean read GetWordWrap write SetWordWrap;

    property TextColor: TAlphaColor read GetTextColor write SetTextColor;
    property PressedTextColor: TAlphaColor read FPressedTextColor
      write SetPressedTextColor;

    property TextShadowColor: TAlphaColor read GetTextShadowColor
      write SetTextShadowColor;
  ...
  public
    ...
    function FindObject(const AName: string): TListItemDrawable;
      deprecated 'Use FindDrawable';

    function DrawableByName(const AName: string): TListItemDrawable;
    function ObjectByName(const AName: string): TListItemDrawable;
      deprecated 'Use DrawableByName';

    property Count: Integer read GetCount;
    property Drawables[const Index: Integer]: TListItemDrawable
      read GetObject; default;
  ...
  IListViewController = interface
    ...
    property DeleteModeTransitionAlpha: Single
      read GetDeleteModeTransitionAlpha;
  ...
  TListViewModeFlag = (Edit, Enabled, Visible, Deletion, PullRefresh, Buttons,
    Search, SearchOnTop, PullRefreshWait, SwipeDelete);
  ...
  IListViewDrawableShim = interface
    ...
    function CalcAppearanceBounds(const AValue: TRect;
      const CurrentBounds: TRectF): TRectF;
  end;
  ...
  IListViewAdapter = interface
    ...
    property OnItemsCouldHaveChanged: TNotifyEvent
  ...
  TAfterItemAddedNotify = procedure(Sender: IListViewEditor;
    const Item: TListItem) of object;
  TBeforeItemDeletedNotify = procedure(Sender: IListViewEditor;
  ...
  IListViewEditor = interface
    ...
    property OnBeforeItemDeleted: TBeforeItemDeletedNotify
      write SetBeforeItemDeleted;
    /// <summary>Notification after item is deleted</summary>
    property OnAfterItemDeleted: TAfterItemDeletedNotify
      write SetAfterItemDeleted;
  end;
  ...
  IListViewExtrasProvider = interface
    ...
    procedure SetItemData(const Index: Integer; const DataIndex: string;
      const AValue: TValue);
    ...
    property Data[const Index: Integer; const DataIndex: string]: TValue
      read GetItemData write SetItemData;
    /// <summary>Tag of item at Index</summary>
    property Tag[const Index: Integer]: NativeInt read GetItemTag
      write SetItemTag;
    /// <summary>True if item at Index has Indexed data DataIndex</summary>
    property HasData[const Index: Integer; const DataIndex: string]: Boolean
      read GetHasData;
  end;
  ...
  IListViewFilterable = interface
    ...
    property Filter: TFilterPredicate read GetFilterPredicate
      write SetFilterPredicate;
  end;

implementation

uses
  System.RTLConsts, System.UIConsts, System.Math, System.Messaging,
  System.TypInfo, FMX.Consts,
  FMX.Styles,
  FMX.Platform;
...
function Blend(const Color1, Color2: TAlphaColor; const Alpha: Single)
  : TAlphaColor;

  function BlendComponent(const Value1, Value2: Integer; const Alpha: Single)
    : Integer; inline;
  begin
...
procedure TListItemDrawable.DoUpdateValuesFromResources(const Resources
  : TListItemStyleResources; const Purpose: TListItemPurpose);
begin
end;
...
procedure TListItemDrawable.SetOneDimension(const Index: Integer;
  const Value: Single);
var
  NewValue: Single;
begin
...
procedure TListItemDrawable.SetInvalidateCallback(const Callback
  : TListItemCallback);
begin
  FCallback := Callback;
end;
...
procedure TListItemDrawable.UpdateValuesFromResources(const Resources
  : TListItemStyleResources; const Purpose: TListItemPurpose);
begin
...
procedure TListItemDrawable.CalculateLocalRect(const DestRect: TRectF;
  const SceneScale: Single; const DrawStates: TListItemDrawStates;
...
begin
  ...
  begin
    AlignValue(FAlign, FLocalRect.Left, FLocalRect.Right, FWidth, DestRect.Left,
      DestRect.Right, LOffset.X);
  end
  else
  begin
  ...
  begin
    AlignValue(FVertAlign, FLocalRect.Top, FLocalRect.Bottom, FHeight,
      DestRect.Top, DestRect.Bottom, LOffset.Y);
  end
  else
  begin
...
function TListItemDrawable.MouseDown(const Button: TMouseButton;
  const Shift: TShiftState; const MousePos: TPointF): Boolean;
begin
  Result := False;
end;

procedure TListItemDrawable.MouseMove(const Shift: TShiftState;
  const MousePos: TPointF);
begin
end;

procedure TListItemDrawable.MouseUp(const Button: TMouseButton;
  const Shift: TShiftState; const MousePos: TPointF);
begin
end;
...
procedure TListItemText.DoUpdateValuesFromResources(const Resources
  : TListItemStyleResources; const Purpose: TListItemPurpose);
var
  ResourceFont: TFont;
begin
  ...
  begin
    ...
    begin
      ...
      if (TStyleResource.TextColor in FStyleValuesNeedUpdate) and
        (Resources.HeaderTextColor > 0) then
    ...
    begin
      ...
      if (TStyleResource.TextColor in FStyleValuesNeedUpdate) and
        (Resources.DetailTextColor > 0) then
    ...
    begin
      ...
      if (TStyleResource.TextColor in FStyleValuesNeedUpdate) and
        (Resources.DefaultTextColor > 0) then
...
procedure TListItemText.CalculateLocalRect(const DestRect: TRectF;
  const SceneScale: Single; const DrawStates: TListItemDrawStates;
  const Item: TListItem);
var
  NewRect: TRectF;
...
begin
  ...
  begin
    ...
    if Item.Controller <> nil then
      FLocalRect.Right := Min(FLocalRect.Right,
        Item.Controller.GetClientMargins.Right);
  end
  else
    inherited;
  ...
  if (TListItemDrawState.Deleting in DrawStates) and (Item.Controller <> nil)
  then
    FLocalRect.Right := Min(FLocalRect.Right,
      Item.Controller.GetItemDeleteCutoff(Item));
end;

procedure TListItemText.Render(const Canvas: TCanvas;
...
begin
  ...
  begin
    ...
    try
      ...
      FTextLayout.MaxSize := TPointF.Create(FLocalRect.Width,
        FLocalRect.Height);
...
procedure TListItemDummy.Render(const Canvas: TCanvas;
  const DrawItemIndex: Integer; const DrawStates: TListItemDrawStates;
  const Resources: TListItemStyleResources;
  const Params: TListItemDrawable.TParams; const SubPassNo: Integer = 0);
begin
...
procedure TListItemImage.CalculateLocalRect(const DestRect: TRectF;
  const SceneScale: Single; const DrawStates: TListItemDrawStates;
  const Item: TListItem);
var
  NewRect: TRectF;
...
procedure TListItemImage.FitInto(const Bitmap: TBitmap;
  var InputRect, DestinationRect: TRectF);
...
procedure TListItemImage.Render(const Canvas: TCanvas;
  const DrawItemIndex: Integer; const DrawStates: TListItemDrawStates;
  const Resources: TListItemStyleResources;
  const Params: TListItemDrawable.TParams; const SubPassNo: Integer = 0);
var
...
{$ENDIF}
  ...
  if (ImageSource = TImageSource.ImageList) and (Params.Images <> nil) then
    Bitmap := Params.Images.Bitmap(TPointF(FLocalRect.Size) * Canvas.Scale,
      FImageIndex)
  else
    Bitmap := GetBitmap;
...
function TListItemSimpleControl.MouseDown(const Button: TMouseButton;
  const Shift: TShiftState; const MousePos: TPointF): Boolean;
begin
  Result := False;

  if (Button = TMouseButton.mbLeft) and FEnabled and PointInLocalRect(MousePos)
    and IsClickOpaque then
  begin
    FPressed := True;
    FMouseOver := True;
...
procedure TListItemSimpleControl.MouseMove(const Shift: TShiftState;
  const MousePos: TPointF);
begin
...
procedure TListItemSimpleControl.MouseUp(const Button: TMouseButton;
  const Shift: TShiftState; const MousePos: TPointF);
var
  ShouldClick: Boolean;
begin
...
procedure TListItemAccessory.CalculateLocalRect(const DestRect: TRectF;
  const SceneScale: Single; const DrawStates: TListItemDrawStates;
  const Item: TListItem);
var
  NewRect: TRectF;
begin
  ...
  begin
    ...
    if FController <> nil then
      NewRect.Offset(FController.GetItemEditOffset(Item) *
        FController.EditModeTransitionAlpha, 0);
...
procedure TListItemAccessory.Render(const Canvas: TCanvas;
  const DrawItemIndex: Integer; const DrawStates: TListItemDrawStates;
  const Resources: TListItemStyleResources;
  const Params: TListItemDrawable.TParams; const SubPassNo: Integer = 0);
var
  ...
  begin
    ...
    begin // Apply crossfading between selected and non-selected states.
      ...
      begin
        ...
        if Image <> nil then
          Image.DrawToCanvas(Canvas, FLocalRect,
            TempOpacity * (1 - SelectedAlpha));
      end;
...
begin
  ...
  begin
    if FTimerService <> nil then
      FTransitionAlpha :=
        Max(1 - (Abs(FTimerService.GetTick - FTransitionStartTicks) /
...
function TListItemGlyphButton.MouseDown(const Button: TMouseButton;
  const Shift: TShiftState; const MousePos: TPointF): Boolean;
begin
  if not FClickOnSelect then
...
procedure TListItemGlyphButton.CalculateLocalRect(const DestRect: TRectF;
  const SceneScale: Single; const DrawStates: TListItemDrawStates;
  const Item: TListItem);
var
  NewRect: TRectF;
...
procedure TListItemGlyphButton.Render(const Canvas: TCanvas;
  const DrawItemIndex: Integer; const DrawStates: TListItemDrawStates;
  const Resources: TListItemStyleResources;
  const Params: TListItemDrawable.TParams; const SubPassNo: Integer = 0);
var
...
begin
  ...
  if TListItemDrawState.EditMode in DrawStates then
    TempOpacity := TempOpacity *
      Max(0.1, FController.EditModeTransitionAlpha * 2 - 1)
  else
    Exit;
...
{$ENDIF}
  ...
  begin
    ...
    if (RenderGlyphButtonType = TGlyphButtonType.Delete) and
      (not FController.IsDeleteModeAllowed) then
      RenderGlyphButtonType := TGlyphButtonType.Checkbox;
    case RenderGlyphButtonType of
      TGlyphButtonType.Add:
        begin
          if (not ReallyPressed) and
            (Resources.ButtonAddItemStyleImage.Normal <> nil) then
            Resources.ButtonAddItemStyleImage.Normal.DrawToCanvas(Canvas,
              LocRect, TempOpacity);

          if ReallyPressed and (Resources.ButtonAddItemStyleImage.Pressed <> nil)
          then
            Resources.ButtonAddItemStyleImage.Pressed.DrawToCanvas(Canvas,
              LocRect, TempOpacity);
        end;
      ...
      TGlyphButtonType.Delete:
        begin
          ...
          if Resources.ButtonDeleteItemStyleImage.Normal <> nil then
            Resources.ButtonDeleteItemStyleImage.Normal.DrawToCanvas(Canvas,
              LocRect, TempOpacity);

          if Resources.ButtonDeleteItemStyleImage.Pressed <> nil then
          begin
            if FTransitionAlpha <= 0 then
              Resources.ButtonDeleteItemStyleImage.Pressed.DrawToCanvas(Canvas,
                LocRect, TempOpacity)
            else
            begin
      ...
      TGlyphButtonType.Checkbox:
        begin
          if (not FChecked) and (Resources.ButtonCheckboxStyleImage.Normal
            <> nil) then
            Resources.ButtonCheckboxStyleImage.Normal.DrawToCanvas(Canvas,
              LocRect, TempOpacity);

          if FChecked and (Resources.ButtonCheckboxStyleImage.Pressed <> nil)
          then
            Resources.ButtonCheckboxStyleImage.Pressed.DrawToCanvas(Canvas,
              LocRect, TempOpacity);
        end;
    end;
  end;
...
procedure TListItemTextButton.CalculateLocalRect(const DestRect: TRectF;
  const SceneScale: Single; const DrawStates: TListItemDrawStates;
  const Item: TListItem);
{$IFDEF MSWINDOWS}
var
...
procedure TListItemTextButton.DoUpdateValuesFromResources(const Resources
  : TListItemStyleResources; const Purpose: TListItemPurpose);
var
...
procedure TListItemTextButton.SetInvalidateCallback(const Callback
  : TListItemCallback);
begin
  inherited;
...
procedure TListItemTextButton.Render(const Canvas: TCanvas;
  const DrawItemIndex: Integer; const DrawStates: TListItemDrawStates;
  const Resources: TListItemStyleResources;
...
{$ENDIF}
    ...
    begin
      case FButtonType of
        TTextButtonType.Normal:
          begin
            if (not ReallyPressed) and
              (Resources.ButtonNormalStyleImage.Normal <> nil) then
              Resources.ButtonNormalStyleImage.Normal.DrawToCanvas(Canvas,
                FLocalRect, FTintColor, TempOpacity);

            if ReallyPressed and (Resources.ButtonNormalStyleImage.Pressed
              <> nil) then
              Resources.ButtonNormalStyleImage.Pressed.DrawToCanvas(Canvas,
                FLocalRect, FTintColor, TempOpacity);
          end;
        ...
        TTextButtonType.Delete:
          begin
            if (not ReallyPressed) and
              (Resources.ButtonDeleteStyleImage.Normal <> nil) then
              Resources.ButtonDeleteStyleImage.Normal.DrawToCanvas(Canvas,
                FLocalRect, FTintColor, TempOpacity);

            if ReallyPressed and (Resources.ButtonDeleteStyleImage.Pressed
              <> nil) then
              Resources.ButtonDeleteStyleImage.Pressed.DrawToCanvas(Canvas,
                FLocalRect, FTintColor, TempOpacity);
          end;
      end;
    end;
  ...
  begin
    ...
    FTextDrawable.Render(Canvas, DrawItemIndex, DrawStates, Resources,
      Params, 0);
  end;
end;
    ...
    try
      for Index := 0 to ChildrenCount - 1 do
        if (Children[Index] is TControl) and
          (TControl(Children[Index]).Visible or
          ((not TControl(Children[Index]).Visible) and
          (csDesigning in ComponentState) and (not TControl(Children[Index])
          .Locked))) then
        begin
...
procedure TListItemEmbeddedControl.Render(const Canvas: TCanvas;
  const DrawItemIndex: Integer; const DrawStates: TListItemDrawStates;
  const Resources: TListItemStyleResources;
  const Params: TListItemDrawable.TParams; const SubPassNo: Integer = 0);
var
...
begin
  ...
  FScene.FLayoutSize := TPoint.Create(Trunc(Canvas.Width),
    Trunc(Canvas.Height));

  BoundsPos := FLocalRect.TopLeft;
  BoundsPos := BoundsPos + Params.ParentAbsoluteRect.TopLeft;

  FContainer.SetBounds(BoundsPos.X, BoundsPos.Y, FLocalRect.Width,
    FLocalRect.Height);
...
begin
  ...
  if (Control <> nil) and (Control.GetObject is TControl) and
    (Control.GetObject <> FContainer) then
...
begin
  ...
  FView.Callback :=
      procedure(View: TListItemView; Drawable: TListItemDrawable;
      Op: TListItemCallbackOp)
    begin
      case Op of
...
begin
  Result := Format('%s@%x Purpose=%s |View|=%d',
    [GetClassName, NativeUInt(Self), GetPurpose, GetViewCount]);
end;
...
function TListItem.MouseDown(const Button: TMouseButton;
  const Shift: TShiftState; const MousePos: TPointF): Boolean;
var
  I: Integer;
begin
...
procedure TListItem.MouseMove(const Shift: TShiftState;
  const MousePos: TPointF);
var
  I: Integer;
begin
...
procedure TListItem.MouseUp(const Button: TMouseButton;
  const Shift: TShiftState; const MousePos: TPointF);
var
  I: Integer;
begin
...
begin
  ...
  for I := 0 to FView.Count - 1 do
    if (FView[I] <> nil) and FView[I].Visible and
      (FView[I] is TListItemGlyphButton) then
    begin
...
procedure TListItemView.Insert(const Index: Integer;
  const Value: TListItemDrawable);
begin
...
constructor TListItemStyleResources.Create(const Source
  : TListItemStyleResources);
var
  I: TAccessoryType;
begin
  FOwnsObjects := False;
...
begin
  AddEnumElementAliases(TypeInfo(TImageScalingMode),
    ['smStretchWithAspect', 'smOriginal', 'smStretch']);
end;
...
const
  PurposeString: array [TListItemPurpose] of string = ('None', 'Header',
    'Footer');
begin
