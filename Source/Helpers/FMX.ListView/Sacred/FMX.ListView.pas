...
begin
  // *** ZuBy
  { DONE : Иванов В.И. Испраление для работы удаления под Windows }
  Result := True;
//{$IFNDEF MSWINDOWS}
//    True
//{$ELSE}
...
begin
  ...
  if FTapSelectItemIndex <> -1 then
    ...
    //DONE -oSACRED: Заблокирована анимация тащи обнови
...
//    FDeleteLayout.Width := DefaultDeleteButtonWidth;
  ...
  begin
    ...
    FDeleteButton.Margins.Bottom := 10;
    FDeleteButton.Margins.Right := 5;
    FDeleteButton.Stored := False;
    FDeleteButton.Locked := True;
    FDeleteButton.Align := TAlignLayout.MostRight;
    FDeleteButton.Width := DefaultDeleteButtonWidth-5;
  ...
  begin
    ...
    begin
      ...
      if FHorizontal then
        ...
        Trigger := (GetPullRefreshStrokeWidth-20 >= Width){DONE:SACRED скролл коррект низ} OR (GetPullRefreshStrokeWidth+20 <= Width);
        { TODO : Добавить определение нижней граници. Опредиление работает но срабатывает лишний раз }
      // *** ZuBy
    end;
...
begin
  ...
  //DONE -oSACRED: Убрано авто скрытие полосы прокрутки
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            FBrush.Color := TAlphaColorRec.Null;
            FBrush.Kind := TBrushKind.None;
          end;
          // *** ZuBy
        end;
...
begin
  ...
  begin
    ...
    begin
      if HasTouchTracking and ((FMouseClickDelta.X *-1) > MinSwypeThreshold) and (FCanSwipeDelete or FCanSwipeDirection)
...
begin
  ...
  FTransparentSeparator := True;
  FTransparentItems := True;
  TransparentHeaders := True;
