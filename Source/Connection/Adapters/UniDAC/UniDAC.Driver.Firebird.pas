...
type
  TUniDAC_FirebirdConfigurator = class(TInterfacedObject, IDriverConfigurator)
  public
...
uses
  ...
  System.Math,
  Uni,
  InterBaseUniProvider; { Firebird e InterBase compartilham o provider }

function TUniDAC_FirebirdConfigurator.DriverName: TDriver;
begin
  Result := TDriver.Firebird;
end;

procedure TUniDAC_FirebirdConfigurator.Configure(const AConn: TObject; const AParams: TConnectionParams);
var
  Conn: TUniConnection;
begin
  Conn := AConn as TUniConnection;
  Conn.ProviderName := 'InterBase';   { UniDAC usa o mesmo provider para Firebird e InterBase }
  ...
  Conn.SpecificOptions.Values['InterBase.ServerType'] := 'Firebird';
  ...
  begin
    ...
    if AParams.ExtraArgs.ContainsKey('SQLDialect') then
      Conn.SpecificOptions.Values['InterBase.SQLDialect'] := AParams.ExtraArgs['SQLDialect'];
  end;
end;
