...
type
  TdbExpress_FirebirdConfigurator = class(TInterfacedObject, IDriverConfigurator)
  public
...
function TdbExpress_FirebirdConfigurator.DriverName: TDriver;
begin
  Result := TDriver.Firebird;
end;

procedure TdbExpress_FirebirdConfigurator.Configure(const AConn: TObject; const AParams: TConnectionParams);
var
  Conn: TSQLConnection;
  Port: Integer;
begin
  ...
  Conn.ConnectionName := 'DBFirebird';
  Conn.DriverName     := 'Firebird';
  Conn.KeepConnection := True;
  Conn.LoginPrompt    := False;
  Conn.GetDriverFunc  := 'getSQLDriverFIREBIRD';
  ...
  begin
    ...
    if AParams.ExtraArgs.ContainsKey('SQLDialect') then
      Conn.Params.Values['SQLDialect'] := AParams.ExtraArgs['SQLDialect'];
