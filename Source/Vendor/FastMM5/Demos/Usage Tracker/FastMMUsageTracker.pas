...
Description:
 ...
 - Shows FastMM5 allocation usage
...
uses
  ...
  Buttons, ComCtrls, Menus, FastMM5;
...
type
  ...
  private
    ...
    FMemoryManagerState, FPrevMemoryManagerState: TFastMM_MemoryManagerState;
...
var
  ...
  begin
    ...
    sgBlockStatistics.RowCount := FMemoryManagerState.SmallBlockTypeCount + 4;
    sgBlockStatistics.Cells[0, FMemoryManagerState.SmallBlockTypeCount + 1] := 'Medium Blocks';
    sgBlockStatistics.Cells[0, FMemoryManagerState.SmallBlockTypeCount + 2] := 'Large Blocks';
    sgBlockStatistics.Cells[0, FMemoryManagerState.SmallBlockTypeCount + 3] := 'Overall';
    for LInd := 0 to FMemoryManagerState.SmallBlockTypeCount - 1 do
    begin
    ...
    {-----------Medium blocks---------}
    LInd := FMemoryManagerState.SmallBlockTypeCount + 1;
    ...
    {----------Large blocks----------}
    LInd := FMemoryManagerState.SmallBlockTypeCount + 2;
    ...
    if FMemoryManagerState.ReservedLargeBlockAddressSpace > 0 then
      ...
      sgBlockStatistics.Cells[4, LInd] := 'N/A';
    {-----------Overall--------------}
    LInd := FMemoryManagerState.SmallBlockTypeCount + 3;
...
begin
  ...
  try
    // retrieve FastMM info

    FastMM_GetMemoryManagerState(FMemoryManagerState);
    FastMM_GetMemoryMap(TMemoryMap(FMemoryMapEx));

    // Update FastMM Graph with EXE & DLL locations
