...
type
{$IFDEF VER16P}
  [ComponentPlatformsAttribute(pidWin32 or pidWin64 or pidOSX32{$IFDEF VER26P} or pidOSX64{$ENDIF})]
{$ENDIF}
...
begin
  ...
  {$IFDEF CLR}
    ISQLConnection_SetOption(SQLConnection, TSQLConnectionOption(coUseQuoteChar), Integer(UpperCase(trim(Params.Values[SUseQuoteChar])) = 'TRUE'));
  {$ELSE}
    SQLConnection.SetOption(TSQLConnectionOption(coUseQuoteChar), Integer(UpperCase(trim(Params.Values[SUseQuoteChar])) = 'TRUE'));
  {$ENDIF}
