...
{ TBaseAsyncResult }
  ...
  private type
    TAsyncFlag = (Completed, Synchronous, ForceSize = SizeOf(Integer) * 8 - 1); // make the size the same as Integer.
...
begin
  ...
  begin
    ...
    inherited Write(FBuffer^, Longint(FBufEndPos - FBufStartPos));
    FModified := False;
  end;
  if ReRead then
  begin
        ...
        begin
          ...
          begin
            if Supports(LList[J], IEditLinkObserver, LEditObserver) and not LEditObserver.IsReadOnly then
              //raise EObserverException.CreateRes(@sObserverMultipleSingleCast); {Modificado por Nickson em 21/12/2016 às 04:53, preciso estudar muito mais isso...}
          end;
        end
        else
