...
type
  ...
  public
    { Public declarations }
    class function ToSQLDateTime(Text: String): String;
...
type
  ...
  public
    ...
    class function ToSQLDateTime(Text: Variant): String;
...
class function TArrayHelper.ToSQLDateTime(Text: String): String;
var
  DT: TDateTime;
  Return: String;
begin
  try
    DT := StrToDateTime(VarToStr(Text));
    try
      if TTimeDate.IsValidTime(Text) then
        Return := FormatDateTime('hh:nn:ss', DT)
      else if TTimeDate.IsValidDate(Text) then
...
begin
  ...
  // Valida Data e/ou Date/Hora
    ...
    Result := TString.Quote(Self.ToSQLDateTime(Value), Quote)
  else
  begin
    if Value = #0 then
      if Quote = #34 then
...
begin
  ...
  S := System.StrUtils.IfThen(Prettify, EOL, EmptyStr);
  for I := 0 to Count - 1 do
    Result := Result + LTag + Key[I] + RTag + TArrayVariantHelper.VarToStr(ValuesAtIndex[I], EmptyStr) + CTag + Key[I] + RTag + S;
  Result := TString.RemoveLastEOL(Result);
end;
...
begin
  ...
  Result := Result + LBracket + S1;
  for I := 0 to Count - 1 do
    Result := Result + S2 + DQuote + Key[I] + DQuote + Colon + TArrayVariantHelper.VarToStr(ValuesAtIndex[I], DQUOTE) + Comma + S1;
  Result := TString.RemoveLastComma(Result);
  Result := Result + RBracket;
end;
...
class function TArrayVariantHelper.ToSQLDateTime(Text: Variant): String;
var
  DT: TDateTime;
  Return: String;
begin
...
begin
  ...
  for I := 0 to Count - 1 do
  begin
    Result := Result + Key[I] + S1 + TArrayVariantHelper.VarToStr(ValuesAtIndex[I].AsVariant) + S2;
    Result := TString.RemoveLastComma(Result);
  end;
end;
...
begin
  ...
  S := System.StrUtils.IfThen(Prettify, EOL, EmptyStr);
  for I := 0 to Count - 1 do
    Result := Result + LTag + Key[I] + RTag + TArrayVariantHelper.VarToStr(ValuesAtIndex[I].AsVariant, EmptyStr) + CTag + Key[I] + RTag + S;
  Result := TString.RemoveLastEOL(Result);
end;
...
begin
  ...
  Result := Result + LBracket + S1;
  for I := 0 to Count - 1 do
    Result := Result + S2 + DQuote + Key[I] + DQuote + Colon + TArrayVariantHelper.VarToStr(ValuesAtIndex[I].AsVariant, DQUOTE) + Comma + S1;
  Result := TString.RemoveLastComma(Result);
  Result := Result + RBracket;
end;
